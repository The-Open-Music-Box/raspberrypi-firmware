...
/**
 * Refactored Socket Service for TheOpenMusicBox - Testable Architecture
 *
 * This version uses dependency injection for better testability.
 * No singleton pattern - instances can be created with mock dependencies for testing.
 *
 * @example
 * // Production usage:
 * const socketService = SocketServiceFactory.create()
 *
 * // Testing usage:
 * const mockSocket = createMockSocket()
 * const mockLogger = createMockLogger()
 * const socketService = new SocketService({ socket: mockSocket, logger: mockLogger })
 */

import type { Socket } from 'socket.io-client'
import type { NativeWebSocketClient } from './nativeWebSocket'
import type {
  StateEventEnvelope,
  OperationAck,
  PlayerState,
...
/**
 * Socket factory interface for creating socket connections
 */
export interface ISocketFactory {
  createSocketIO: (url: string, options: any) => Socket
  createNativeWebSocket: (config: any) => NativeWebSocketClient
}

/**
 * Dependencies for SocketService constructor
 */
...
/**
 * SocketService - Refactored for testability with dependency injection
 */
export class SocketService {
  ...
  private readonly config: ISocketConfig
  ...
  constructor(deps: SocketServiceDependencies) {
    ...
    this.logger.info('Initializing SocketService with dependency injection')
  ...
  private initializeSocket(): void {
    ...
    this.isESP32Mode = this.config.options.path === '/ws'

    this.logger.info(`🔊🎵 Initializing ${this.isESP32Mode ? 'ESP32 (Native WebSocket)' : 'RPI Backend (Socket.IO)'} connection to: ${this.config.url}`)

    if (this.isESP32Mode) {
      // Use native WebSocket for ESP32
      this.socket = this.socketFactory.createNativeWebSocket({
        url: this.config.url,
        path: '/ws',
      ...
      this.socket = this.socketFactory.createSocketIO(this.config.url, {
        ...this.config.options,
      ...
      if (!(this.socket as Socket).connected) {
        this.logger.info('🔊🎵 Manually connecting Socket.IO...')
  ...
  private setupNativeWebSocketHandlers(): void {
    ...
    ws.on('internal:connection_changed', (data: any) => {
      ...
      if (data.connected) {
        this.logger.info('🔊🎵 ✅ NATIVE WEBSOCKET CONNECTED TO ESP32!', {
          url: this.config.url
        })
        ...
        this.logger.warn(`Native WebSocket disconnected: ${data.reason}`)
    ...
    ws.on('internal:connection_error', (data: any) => {
      this.logger.error('🔊🎵 ❌ NATIVE WEBSOCKET CONNECTION ERROR:', data.error)
    ...
    ws.on('internal:connection_failed', (data: any) => {
      this.logger.error('Native WebSocket connection failed:', data)
    ...
    stateEvents.forEach(eventType => {
      ws.on(eventType, (envelope: any) => {
        ...
        // Forward envelope to local handlers
  ...
  private setupConnectionHandlers(): void {
    ...
    socketIO.on('connect', () => {
      this.logger.info('🔊🎵 ✅ SOCKET.IO CONNECTED TO RPI BACKEND!', {
        url: this.config.url,
        id: socketIO.id
      })
    ...
    socketIO.on('reconnect', (attemptNumber) => {
      this.logger.info(`Socket reconnected after ${attemptNumber} attempts`)
      this.reconnectionAttempts = 0
  ...
  private setupStandardizedEventHandlers(): void {
    const socketIO = this.socket as Socket
    this.logger.info('🔧 Setting up standardized Socket.IO event handlers...', { url: this.config.url })

    // Connection status handling
    socketIO.on('connection_status', (data) => {
      this.logger.debug('Received connection status:', data)
      this.connectionStatus.ready = true
      this.connectionStatus.serverId = data.sid
      this.connectionStatus.lastSeq = data.server_seq
      this.connectionStatus.serverTime = data.server_time
    ...
    socketIO.on('ack:join', (data) => {
      this.logger.debug(`Room join ack: ${data.room} - ${data.success}`)
    ...
    socketIO.on('ack:leave', (data) => {
      this.logger.debug(`Room leave ack: ${data.room} - ${data.success}`)
      if ((data as { success?: boolean }).success) {
        this.subscribedRooms.delete(data.room)
      }
      this.emitLocal('ack:leave', data)
    ...
    socketIO.on('ack:op', (data: OperationAck) => {
      this.logger.debug(`Operation ack: ${data.client_op_id} - ${data.success}`)
      this.resolveOperation(data.client_op_id, data)
      this.emitLocal('ack:op', data)
    })
    ...
    socketIO.on('sync:complete', (data) => {
      this.logger.debug('Sync complete received')
      this.emitLocal('sync:complete', data)
    })

    socketIO.on('sync:error', (data) => {
      this.logger.error('Sync error received:', data)
  ...
  private setupStateEventHandler(eventType: string): void {
    const socketIO = this.socket as Socket
    this.logger.debug(`🔧 Setting up handler for: ${eventType}`)
    ...
    socketIO.on(eventType, (rawData) => {
      try {
        ...
        if (eventType.includes('player') || eventType.includes('playlist_created') || eventType === 'state:track_position') {
          if (eventType === 'state:track_position') {
            if (!this._firstTrackPositionLogged) {
              this.logger.info(`🎯 FIRST ${eventType} received via WebSocket!`, rawData)
              this._firstTrackPositionLogged = true
            }
          } else {
            this.logger.info(`🔊🎵 WebSocket event received: ${eventType}`, {
              eventType,
              rawData,
        ...
        if (eventType === 'state:track_position' && !this._envelopeLogged) {
          this.logger.info(`📦 Track position envelope:`, envelope)
          this._envelopeLogged = true
        }

        // Skip sequence ordering for track position events
        if (eventType === 'state:track_position') {
  ...
  private processEvent(envelope: StateEventEnvelope): void {
    this.logger.debug(`Processing event: ${envelope.event_type} (seq: ${envelope.server_seq})`)
  ...
  private dispatchDOMEvent(eventType: string, envelope: StateEventEnvelope): void {
    try {
      ...
      if (eventType === 'state:track_position' && !this._domDispatchLogged) {
        this.logger.info(`🎯 FIRST state:track_position dispatched to DOM!`, { eventType, envelope })
        this._domDispatchLogged = true
      }

      // Enhanced logging for debugging
      if (eventType.includes('player') || eventType.includes('playlist_created')) {
        this.logger.info(`🔊 DOM event dispatched: ${eventType}`, {
          eventType,
          data: envelope.data,
          envelopeKeys: Object.keys(envelope)
        })
      } else {
        this.logger.debug(`DOM event dispatched: ${eventType}`)
      }
    } catch (error) {
      this.logger.error(`Failed to dispatch DOM event ${eventType}:`, error)
    }
  }
  ...
  async joinRoom(room: string): Promise<void> {
    ...
    const promise = new Promise<void>((resolve, reject) => {
      ...
      const successHandler = (data: unknown) => {
        if ((data as { room?: string }).room === room) {
      ...
      // Emit specific room join events
      if (room === 'playlists') {
  ...
  async leaveRoom(room: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Emit specific room leave events
      if (room === 'playlists') {
  ...
  private async resubscribeToRooms(): Promise<void> {
    ...
    for (const room of rooms) {
      try {
        ...
        this.logger.debug(`Resubscribed to room: ${room}`)
      } catch (error) {
        this.logger.error(`Failed to resubscribe to room ${room}:`, error)
      }
    }
  }
  ...
  private performPostConnectionSync(): void {
    if (!this.isConnected()) {
      this.logger.warn('Cannot perform post-connection sync - not connected')
      return
    }

    this.logger.info('🔄 Performing post-connection synchronization...')
    ...
    try {
      ...
      this.logger.info('✅ Post-connection sync request sent')
    } catch (error) {
      this.logger.error('❌ Error in post-connection sync:', error)
    }
  }
