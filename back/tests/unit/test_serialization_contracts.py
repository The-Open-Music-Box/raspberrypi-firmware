...
class TestSerializationContracts:
    ...
    def test_track_serialization_includes_required_fields(self, sample_track):
        ...
        required_fields = [
            'id',
            'track_number',  # Per OpenAPI contract v4.0.0 - CRITICAL
            'title',
            'filename',
            'file_path',
            'duration_ms',
        ...
        # Per OpenAPI contract v4.0.0, 'track_number' field should exist (no mapping needed)
    ...
    def test_track_serialization_field_types(self, sample_track):
        ...
        # Type validation per OpenAPI contract v4.0.0
    ...
    def test_track_serialization_with_property_field(self, sample_track):
        """Test that @property field 'track_number' is included despite being a property."""
        result = UnifiedSerializationService.serialize_track(sample_track, format="api")

        # The 'track_number' field is a @property on Track model
        # It should be explicitly added by serialization service
        assert 'track_number' in result, \
            "Serialization MUST include @property field 'track_number' for frontend compatibility"

        # Verify it returns the correct value
        assert result['track_number'] == sample_track.track_number, \
            "Serialized 'track_number' must match Track.track_number property"

    def test_playlist_serialization_includes_tracks_with_number_field(self, sample_playlist):
        """Test that playlist serialization includes tracks with 'track_number' field."""
        ...
        # Verify each track has the 'track_number' field
        for track in result['tracks']:
            assert 'track_number' in track, \
                f"Track '{track.get('title')}' missing 'track_number' field in playlist serialization"
            assert isinstance(track['track_number'], int), \
                f"Track 'track_number' field must be integer, got {type(track['track_number'])}"

    def test_track_serialization_consistency_across_formats(self, sample_track):
        """Test that different serialization formats maintain 'track_number' field."""
        api_result = UnifiedSerializationService.serialize_track(sample_track, format="api")

        # API format MUST include 'track_number' field
        assert 'track_number' in api_result, "API format must include 'track_number' field"
    ...
    def test_track_serialization_from_dict(self):
        ...
        assert result['track_number'] == 5, "track_number should match track_number from dict"
    ...
    def test_track_serialization_handles_missing_optional_fields(self):
        ...
        assert 'track_number' in result, "Must include 'track_number' even for minimal track"
    ...
    def test_serialization_prevents_silent_field_omission(self, sample_track):
        ...
        assert 'track_number' in result, \
            "REGRESSION CHECK: 'track_number' field must be present " \
            "(Track entity now uses 'track_number' directly per OpenAPI contract v4.0.0)"
    ...
    def test_playlist_serialization_regression_check(self, sample_playlist):
        ...
        # Issue #71: Playlist broadcasts missing 'track_number' field
        # This test ensures it never happens again

        for idx, track in enumerate(result['tracks']):
            assert 'track_number' in track, \
                f"REGRESSION: Track {idx} in playlist missing 'track_number' field. " \
                f"This caused issue #71 where playlists with NFC tags failed to display."

            # Per OpenAPI contract v4.0.0, 'track_number' field must exist
            assert 'track_number' in track, \
                f"Track {idx} must have 'track_number' field per OpenAPI contract v4.0.0"
...
class TestSerializationContractValidation:
    ...
    def test_validate_track_dict_with_valid_track(self):
        ...
        # Valid track now only has 'track_number' per OpenAPI contract v4.0.0
        assert valid_track['track_number'] == 1

    def test_validate_track_dict_detects_missing_number_field(self):
        """Test that we can detect missing 'track_number' field (the bug from issue #71)."""
        # This test documents what the bug looked like - missing 'track_number' field
        buggy_track_without_number = {
            'id': 'buggy-id',
            # 'track_number': 1,  # MISSING - this was the bug!
            'title': 'Buggy Track',
            'filename': 'buggy.mp3',
            'duration_ms': 180000
        }

        # This is what the bug looked like - no 'track_number' field
        assert 'track_number' not in buggy_track_without_number, \
            "This test documents the bug: 'track_number' field was missing"
    ...
    def test_all_serialization_paths_produce_identical_output(self):
        ...
        # Key fields should be present per OpenAPI contract v4.0.0
...
class TestContractRegressionPrevention:
    ...
    def test_track_number_field_always_present_in_api_format(self):
        """Critical test: 'track_number' field MUST be present in API format."""
        track = Track(
            track_number=99,
            title="Critical Test",
        ...
        assert 'track_number' in result, \
            "CRITICAL: 'track_number' field missing. This causes frontend contract violations!"

        assert result['track_number'] == 99, \
            "'track_number' field has wrong value"

    def test_websocket_broadcast_data_includes_number_field(self):
        """Test that WebSocket broadcast data includes 'track_number' field."""
        ...
        # which didn't include 'track_number' field
        assert len(result['tracks']) > 0
        assert 'track_number' in result['tracks'][0], \
            "WebSocket broadcasts MUST include 'track_number' field (issue #71 regression check)"

    def test_nfc_triggered_playlist_load_includes_number_field(self):
        """Regression test: NFC-triggered playlist loads must include 'track_number' field."""
        ...
        assert 'track_number' in track, \
            f"CONTRACT VIOLATION: Track missing required 'track_number' field. " \
