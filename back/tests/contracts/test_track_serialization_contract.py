...
After the refactoring (commit refactor(domain)), Track entity now directly uses 'track_number'
field to align with OpenAPI contract v4.0.0.
"""
...
class TestTrackSerializationContract:
    ...
    def test_track_entity_has_required_fields(self, sample_track, openapi_schema):
        ...
        After the refactoring, Track entity directly uses 'track_number' field
    ...
    def test_track_serialization_field_names_match_contract(self, sample_track, openapi_schema):
        ...
        After refactoring, Track.track_number is a direct dataclass field (not a property),
        so asdict() serializes it correctly as 'track_number'.
        """
        # Serialize track using asdict
        serialized = asdict(sample_track)
        ...
        # Verify 'track_number' field is present (no mapping needed anymore)
        assert 'track_number' in serialized, (
            "Track should have 'track_number' field after serialization"
        )
    ...
    def test_track_number_field_is_dataclass_field(self, sample_track):
        """Test that 'track_number' is a direct dataclass field (not a property).

        After refactoring, Track.track_number is a direct field, so asdict()
        serializes it correctly without any mapping.
        """
        serialized = asdict(sample_track)

        # Track.track_number should be a direct field
        assert hasattr(sample_track, 'track_number'), "Track should have 'track_number' attribute"

        # asdict() should serialize 'track_number' directly
        assert 'track_number' in serialized, "asdict() should serialize 'track_number' field"
        assert serialized['track_number'] == sample_track.track_number, "Serialized track_number should match entity"
    ...
    def test_repository_serialization_is_contract_compliant(self, sample_track, openapi_schema):
        ...
        # Should have 'track_number' directly from asdict()
        assert 'track_number' in track_dict, "Serialized track should have 'track_number' field per contract"
    ...
    def test_track_list_serialization_contract(self, openapi_schema):
        ...
        for i, track_dict in enumerate(serialized_tracks, 1):
            assert 'track_number' in track_dict, f"Track {i} should have 'track_number' field"
            assert track_dict['track_number'] == i, f"Track {i} should have track_number={i}"
    ...
    def test_reorder_tracks_response_contract(self):
        ...
        for i, track_dict in enumerate(serialized, 1):
            assert track_dict['track_number'] == i, (
                f"Reordered track at position {i} should have track_number={i}, got {track_dict.get('track_number')}"
            )
            assert 'track_number' in track_dict, f"Track {i} must have 'track_number' field"

        # Verify no duplicates
        track_numbers = [t['track_number'] for t in serialized]
        assert len(track_numbers) == len(set(track_numbers)), f"Duplicate track numbers found: {track_numbers}"
...
class TestRepositorySerializationIntegration:
    ...
    async def test_pure_sqlite_repository_get_tracks_returns_contract_compliant_data(self):
        ...
        After refactoring, Track entity uses 'track_number' directly, so no mapping needed.
        """
        ...
        with patch('app.src.infrastructure.repositories.pure_sqlite_playlist_repository.get_database_manager') as mock_get_db:
            ...
            # After refactoring, 'track_number' is directly available
            assert 'track_number' in track, (
                "Serialized track should have 'track_number' field required by OpenAPI contract"
            )
